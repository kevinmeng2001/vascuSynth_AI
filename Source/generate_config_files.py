...
def generate_parameter_file(p_num=1,
                            ...
                            random_seed = 0, # MAKE SURE TO CHANGE THIS TO 0 IF UNIQUE TREES WANTED.
                            perf_point = "0 50 50",
...
def generate_and_write_oxygen_demand_gradient(file):
    ...
    percent_of_total = round(random.uniform(0.10, 0.35), 2) # Tumor size
    print('Percent of total volume as Hypoxic Region:', percent_of_total)
    
    # Bottom right corner coordinates
    b_right_corner_x = random.randint(0, vol_dim[0]*0.3) # Kevin - Changed from 0.4 to account for 50% volume tumor
    b_right_corner_y = random.randint(0, vol_dim[1]*0.3)
    b_right_corner_z = random.randint(0, vol_dim[2]*0.3)
    ...
    decrement_x_y_z = (t_left_corner_x - b_right_corner_x) * 0.05 # Kevin - Changed from 0.05 for larger hypoxic region
